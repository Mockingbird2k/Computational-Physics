...
probability=.57
probability_array=np.array([0.58,.583,.587,.59,.593,.595,.596,.597,.598,.599,.60,.603,.607,.61])
global counter
counter=1
...
def color_checker(value):#checks if a color should be considered as color==0 or not(is connected to infinity cluster or not)
...
def real_color(value):# this function is very similar to function 'color_checker' but it returns the real color of a color
    if color_array[int(value)]!=int(value):
...
def big_cluster_finder(real_color_array,area_array,temp):   #in the case,system has percolated it finds the "real color" of the bigest cluster(except the percolated cluster)
...
def mother(plane,x,y,probability):#checks if a random number is less than probability,turns it on(gives birth to it!  LOL)
        temp=random.random()
        if temp<=probability:
...
while k<len(L_array):
    ...
    while m<len(probability_array):
        ...
        n=0
        while n<50:
            area_array=np.zeros(L**2+1)#this array keeps the "area" of a cluster in it.and is evaluated in 'value assign function'
            ...
            Q=Q+temp
            real_color_array=np.arange(L**2+1)#all the connected clusters have the same number and stored in this array
            s=0
            while s<counter:
                if s==real_color(s):
                    h=0
                    while h<=counter:
                        ...
                        h+=1
                s+=1
            (index,max_S)=big_cluster_finder(real_color_array,area_array,temp)   #here the color of biggest cluster('index') and its area max_S is evaluated
            if max_S!=0:   
...
PL_array=np.zeros(len(L_array))  #in 'PL_array' the probability in which 'Rg' get maximized will be stored
i=0
while i<len(L_array):  #we evaluate 'PL_array'
    max_index=np.argmax(ksi_array[i,:])
    PL_array[i]=abs(probability_array[max_index]-0.5927)
    i+=1
...
print('best slope(-1/nou) is:')
bestslope=best_slope(np.log(L_array),np.log(PL_array))
print(bestslope)
print('nou is:')
print(-1/bestslope)
...
fig=plt.figure()
plt.plot(np.log(L_array),np.log(PL_array))
