...
earth = np.zeros(L)
point_scale = 100  # 'point_scale' is a measure of how many points you want to drop every time.calculation is as follows:2^(final_step)*point_scale-point_scale

global final_step  # we get data logarithmic. the number of times we get data is final_step
final_step = 8

global current_step  # 'current_step is created so that we can understand if we have gathered data 'final_step' times or not
current_step = 0

global point_counter  # 'point_counter' counts times which 'graph' is called.so that it can adjust the appropriate  color for scattering
point_counter = 0
...
global ensemble_array  # ensemble_array is actually a 2D array. in every row of it,logarithm of 'NAHAMVARI' in different times is kept(every row is for a different run of program)
...
global height_ensemble_array  # height_ensemble_array is actually a 2D array. in every row of it,height of 'earth' in different times is kept(every row is for a different run of program)
...
def w_cal(earth):  # calculates vianarce of data after a specific step
    w = (np.var(earth)) ** .5
    save_w(w)
