...
def evo(earth, final_point):  # final_point is number of points which 'evo' drops in every cycle
    ...
    if current_step < final_step:
        ...
        while current_point < final_point:
            ...
            min_index = np.argmin((earth[(rand - 1) % L], earth[(rand) % L], earth[(rand + 1) % L]))
            if earth[rand] == earth[(rand - 1 + min_index) % L]:
                earth[rand % L] = earth[rand % L] + 1
            else:
                earth[(rand - 1 + min_index) % L] = earth[(rand - 1 + min_index) % L] + 1
